...
export class CartComponent implements OnInit {
  ...
  public calculateTotal() {
    ...
    this.totalCost = 0;
    const removingIndex = [];
    this.booksInCart.forEach((book,i) => {
      this.totalCost += book.price;
      if (this.bookIds.includes(book.id)) {
        removingIndex.push(i);
        book.quantity = book.quantity ? book.quantity + 1 : 2;
        this.booksInCart.forEach(b=>{
          if(b.id==book.id){
    ...
    this.booksInCart = this.booksInCart.filter((b,i)=>!removingIndex.includes(i));
  }
  ...
  public removeFromCart(book: Book) {
    this.booksInCart = this.booksInCart.filter(b => b.id != book.id);
    this.calculateTotal();
    this.cartService.deleteFromCart(book, this.booksInCart).subscribe();
  }
